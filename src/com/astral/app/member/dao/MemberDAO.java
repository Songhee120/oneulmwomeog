...
public class MemberDAO {
	...
	// 회원번호 조회
	public int getMemberNumber(String mem_id) {
		return sql_session.selectOne("mem.getMemberNumber", mem_id);
	}
	...
	public boolean searchMyPw(String mem_id, String mem_name) {
		...
		mem.put("mem_id", mem_id);
		mem.put("mem_name", mem_name);
		mem.put("mem_pw", encrypt(makeNewPw()));
	...
	// 비밀번호 변경
	public boolean changeMyUpdatePw(String mem_id,String mem_newpw) {
		HashMap<String, String> mem = new HashMap<>();
		mem.put("mem_id", mem_id);
		mem.put("mem_newpw", encrypt(mem_newpw));
		
		return (Integer) sql_session.update("mem.changeMyUpdatePw", mem) == 1;
	}
			
	// 회원탈퇴
	public boolean deleteMyId(String mem_id) {
		 return (Integer) sql_session.delete("mem.deleteMyId", mem_id) == 1;
	}
	// 회원랭크 불러오기
    public int getMemberRank(int mem_no) {
       return sql_session.selectOne("mem.getMemberRank", mem_no);
    }
   
    // 회원 리뷰수 
    public int getMemberReviewCnt(int mem_no) {
       return sql_session.selectOne("mem.getMemberReviewCnt", mem_no);
    }
   
    // 회원 하트수
    public int getMemberHeart(int mem_no) {
       return sql_session.selectOne("mem.getMemberHeart", mem_no);
    }
}
