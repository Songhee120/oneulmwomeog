   ...
   <div id="wrapper">
      ...
      <div id="main">
         ...
         <article id="home" class="panel" style="height:870px;padding:0 2.5em">                  
               <div id="food_div">
                  <div id="food_div1">그렇다면 오늘은 <span id="now_food">???</span> 먹!</div>
                  <div id="food_div2">
	                  <form style="margin:0px;height:100%;" name="select_form">
	                     ...
	                     <div id="food_div2-2">
	                        <select id= "ppl_count" name="ppl_count" class="food_select">
	                              <option value="0">몇 명?</option>
	                              <option value="1">혼자</option>
	                              <option value="2">두명 이상</option>
	                              <option value="3">단체</option>
	                          </select> 
	                          <select name="price" class="food_select">
	                              <option value="0">얼마있니?</option>
	                              <option value="1">만원 이하</option>
	                              <option value="2">만원 이상</option>
	                              <option value="3">가격 무관</option>
	                          </select> 
	                          <select name="starv_level" class="food_select">
	                              <option value="0">배고파?</option>
	                              <option value="1">출출함</option>
	                              <option value="2">배고픔</option>
	                              <option value="3">굶주림</option>
	                          </select>
	                          <select name="distance" class="food_select">
	                     ...
	                     <div id="food_div2-3">
	                     	<input type="button" value="출발 위치" id="start_btn" class="food_btn">	                     
	                     </div>
	                  </form>
                  </div>
               ...
               <section id="food_section" style="margin:0px;">
                  <div class="map_wrap">
               ...
               <form action="${pageContext.request.contextPath}/food/FindRoad.fo" method="post" name="food_form">
...
<script>
	    ...
	    function go_back(){
	    	var check = false;
			if(back_check){
				back_ok = confirm("지금 Home으로 돌아가시면 1시간 동안 이용이 불가능합니다.");
				if(back_ok){
		...
		function location_send(check){
			if(check){
				var queryString = $("form[name=select_form]").serialize();
				$.ajax({
					...
					success:function(result){
						if(result.trim() == "none"){
							...
							$("#food_div2").css("display", "none");
							$("#food_div").css("height", "200px");
							$("#home").css("height", "800px");
							$("#now_food").attr("class", "neon");
							$("#now_food").text(result.trim());
							$("#now_food").css("color", "tomato");
							$("#hidden_div").css("display", "block");	
							//음식을 통해 음식점을 검색
							map_view(result.trim());					
						}						
					},
   ...
   function map_view(food){
	   ...
	   function placesSearchCB(data, status, pagination) {
           ...
           //데이터 결과가 0이라면 검색된 식당이 없으므로 재선택을 할 수 있게 해야 한다.		
			   ...
			   $.ajax({
				   url:contextPath + "/food/FoodReset.fo",
				   type:"get",
				   dataType:"text",
				   success:function(result){
					   if(result.trim() == "ok"){
							alert("해당 음식에 대한 음식점이 없어 음식이 랜덤 서칭됩니다.");
							location_send(true);
					   }
				   },
				   error:function(result){
