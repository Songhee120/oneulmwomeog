      ...
      <div id="main">
         ...
         <article id="my_inform" class="panel">
            ...
            <form id="file" method="post" enctype="multipart/form-data" action="">
					<div class="col-12" style="margin: 0 auto; display: block;">
						<div class="filebox">
							<label for="upload">대표 사진 <input type='file' id='upload'
								...
								<div id='preview'>
									<!-- 미리보기 공간 -->
									<img src="" style="width: 80px; height: 80px;" id="mypageImg">
								</div>
							</label>
						</div>
					</div>
					...
					<div class="col-12" style="margin: 0 auto; display: block;">
						<div style="padding: 0px;">
							<label style="margin: 0px; font-size: 20px; font-weight: bold;">이메일</label>
						</div>
						<div style="padding: 0px;">
							<input type='text' name='input_email'
								style="width: 75%; margin-left: 0px; display: inline-block;" value="${mem_vo.getMem_email()}" readonly />
							<input type="button" value="수정" id="mean_btn" name="email_btn">
						</div>
					</div>
					<div class="col-12" style="margin: 0 auto; display: block;">
...
<script>
	window.onload = function(){
		$.ajax({
			...
			success:function(mem){
				console.log(mem.mem_profile);
				$("#mypageImg").attr("src", contextPath + "/profile/" + mem.mem_profile);
			},
			error:function(mem){
				alert("로딩 실패");
			}
	...
	$("#upload").change(function(){
	    ...
	    $.ajax({
	        url:contextPath + "/member/MemberChangeProfileOk.me",
	        type:"post",
	        data:formData,
	        contentType:false,
	        processData:false        
	...
	//버그
	//1. 값을 수정 안하고 완료를 눌러도 중복이라 뜬다.
	//2. 값을 지웠다가 자신의 원래 닉네임을 적어도 중복이라 뜬다.
	//해결방법
	//회원번호를 같이 날려 입력된 값과 저장된 값이 같다면 그대로 끝낸다.
	//다르다면 다른 닉네임과 중복확인을 하여 없다면 변경된다.
   var nickname;
   $("input[name='nick_btn']").click(function() {
      if ($("input[name='input_nick']").attr("readOnly")) {
         nickname = $("input[name='input_nick']").val();
         $("input[name='input_nick']").attr("readOnly", false);      
         $("input[name='nick_btn']").attr("value", "완료");
      } else {
         var new_nickname = $("input[name='input_nick']").val();
         if(new_nickname == ""){
            alert("변경할 닉네임을 입력해주세요.");
         //값을 수정 안하고 완료를 누르면 자연스럽게 넘어가야한다.
		...
		$("input[name='email_btn']").click(function() {
		if ($("input[name='input_email']").attr("readOnly")) {
			$("input[name='input_email']").attr("readOnly", false);		
			$("input[name='email_btn']").attr("value", "완료");
		} else {
			var new_email = $("input[name='input_email']").val();
			if(new_email == ""){
				alert("변경할 이메일을 입력해주세요.");
			}else{
				$.ajax({
					url : contextPath + "/member/MemberCheckEmailOk.me?new_email=" + new_email,
					type : "get",
					dataType : "text",
					success : function(result){
							//중복된 이메일이 있다.
							if(result.trim() == "not-ok"){
								notice = "이미 중복된 이메일입니다!";							
							}
							//이메일 변경 성공
							else if(result.trim() == "ok"){
								notice = "이메일 변경 완료!";
								$("input[name='input_email']").attr("readOnly", true);
								$("input[name='email_btn']").attr("value", "수정");
							}
							//DB문제
							else{
								notice = "다시 시도해주세요.."
	...
	$("input[name='tel_btn']").click(function() {
		if ($("input[name='input_tel']").attr("readOnly")) {
			...
			var new_tel = $("input[name='input_tel']").val();
			if(new_tel == ""){
