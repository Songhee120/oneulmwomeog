...
<body class="is-preload">
   ...
   <div id="wrapper">
      ...
      <!-- Nav -->
      <nav id="nav">
         <a href="#writereview" class="icon solid fa-user"><span>리뷰쓰기</span></a>      
      </nav>
            ...
            <section id="my_review">
               <div id="my_review_div1">
               	  <div id="my_review_div1-1">
               	  	<span id="my_review_res"><span id="my_res">음식점</span><span id="res_text">에 대해 솔직한 리뷰를 남겨주세요.</span></span>
               	  </div>
                  	...
                  	<select name="res_select">
                  		...
                  		<c:if test="${history_list != null and fn:length(history_list) > 0}">
	                  		<c:forEach var="history" items="${history_list}" varStatus="status">
	                  			<option value="${history.getHistory_no()}">${status.count}.${history.getRes_name()}</option>	      
	                  		</c:forEach>
	                  	</c:if>
                  	</select>
                  </div>             
               </div>
               <form method="post" enctype="multipart/form-data" name="reviewform">
				<!-- 음식점 번호 -->
				...
				<input type="hidden" name="star_grade" value="0">
	               <div id="my_review_div2">
	                  ...
	                  <div id="my_review_div2-2">
	                     <textarea id="my_review_text" name="review_content" placeholder="식사는 즐거우셨나요?맛과 분위기가 궁금해요!"></textarea>
	                  </div>
	               </div>
	               ...
	               <div id="my_review_div4">
	 					<div class="input_wrap" style="height:40px;">
							...
							<label for="input_imgs">사진 업로드<input type="file" id="input_imgs" name="input_imgs[]" multiple="multiple"/></label>							
						</div>
						<div class="imgs_wrap">
							<img id="review_img">
						</div>
...
<script>
	...
	function review_submit(){
		...
		//완성(0)
		frm.action = contextPath + "/review/ReviewWriteOk.re?success=0";
		frm.method = "post";
		frm.submit();
	}
	...
	function review_save(){
		...
		frm.action = contextPath + "/review/ReviewWriteOk.re?success=1";
		frm.submit();
	}
	...
	$("select[name=res_select]").change(function() {
		...
		$("#my_res").text(res_name.substring(2, res_name.length));
	});
</script>
